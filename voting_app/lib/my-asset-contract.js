...
class MyAssetContract extends Contract {
    ...
    async init(ctx) {
        ...
        //create voters
        let voter1 = await new Voter("SN125678", "1992200601154", "Abdou Yaya", "Sadiakhou");
        let voter2 = await new Voter("SN123456", "1092200605154", "Pape Omar", "Diop");
        ...
        for (let i = 0; i < voters.length; i++) {
            if (!voters[i].ballot) {
                ...
                console.log("Ces électeurs ont déjà des bulletins de vote.");
                break;
            }
        }
    ...
    async createVoter(ctx, args) {
        ...
        if (currElections.length === 0) {
            let response = {};
            response.error = "Pas d'élections. Exécutez d'abord la fonction init().";
            return response;
        }
        ...
        let response = `électeur avec CNI ${newVoter.voterId} est mis à jour dans le world state`;
        return response;
    }
    ...
    async castVote(ctx, args) {
        ...
        if (electionExists) {
            ...
            if (voter.ballotCast) {
                let response = {};
                response.error = "Cet électeur a déjà voté !";
                return response;
            }
            ...
            if (
                ...
                if (!votableExists) {
                    let response = {};
                    response.error = "Ce CNI n'existe pas ou n'a pas été enregistré!";
                    return response;
                }
                ...
                let response = {};
                response.error = "L'élection n'est pas encore ouverte!";
                return response;
            }
        } else {
            let response = {};
            response.error = "L'élection ou l'électeur n'existe pas!";
            return response;
        }
    }
